...
class Message {
  ...
  readExec (buffer, length, timed) {
    let [cue, exec] = buffer.toString('ascii', timed ? 0x17 : 0x12, length - 1).split('\0')
    let [number, page] = exec.split('.')
  ...
  readBuffer (buffer) {
    ...
    try {
      ({ length, command } = this.readHeader(buffer))
    } catch (err) {
  ...
  writeExec (buffer, command) {
    let [exec, page] = String(this.data.exec || 0.1).split('.')
    let offset = 0x12
  ...
  writeFader (buffer) {
    ...
    const coarse = (value >= 0 ? value : 0x3fff / 100 * percent) / 128
    const fine = (coarse % 1) * 128
    const [exec, page] = String(this.data.exec || 0.1).split('.')
